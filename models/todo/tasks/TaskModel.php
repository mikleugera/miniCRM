<?php
    ...
    class TaskModel {
        ...
        public function getAllTasksByIdUser ($user_id) {
            try {
               $stmt = $this -> db -> prepare("SELECT * FROM todo_list WHERE finish_date > NOW() AND user_id = :user_id AND status != 'completed' 
        ...
        public function getAllCompletedTasksByIdUser ($user_id) {
            try {
               $stmt = $this -> db -> prepare("SELECT * FROM todo_list WHERE user_id = :user_id AND status = 'completed' 
                                               ORDER BY ABS(TIMESTAMPDIFF(SECOND, NOW(), finish_date))");
               $stmt -> bindParam(':user_id', $user_id, PDO::PARAM_INT);
               $stmt -> execute();
               $todo_list = [];
        ...
        public function getAllExpiredTasksByIdUser ($user_id) {
            try {
               $stmt = $this -> db -> prepare("SELECT * FROM todo_list WHERE finish_date < NOW() AND user_id = :user_id
                                               ORDER BY ABS(TIMESTAMPDIFF(SECOND, NOW(), finish_date))");
               $stmt -> bindParam(':user_id', $user_id, PDO::PARAM_INT);
               $stmt -> execute();
               $todo_list = [];
        ...
        public function createTask($data) {
            try {
                $stmt = $this -> db -> prepare('INSERT INTO todo_list (user_id, title, category_id, status, priority, finish_date) VALUES (?,?,?,?,?,?)');
        ...
        public function getTaskByIdAndByIdUser($id_task, $id_user) {
            try {
                $stmt = $this -> db -> prepare('SELECT * FROM todo_list WHERE id = ? AND user_id = ?');
        ...
        public function updateTask($data) {
            try {
                $stmt = $this -> db -> prepare('UPDATE todo_list SET title = ?, category_id = ?, finish_date = ?, reminder_at =?, 
                                                status = ?, priority = ?, description = ? WHERE id = ?');
                $stmt -> execute([$data['title'], $data['category_id'], $data['finish_date'], $data['reminder_at'], $data['status'],
        ...
        public function deleteTask($id) {
            try {
                $stmt = $this -> db -> prepare('DELETE FROM todo_list WHERE id = ?');
                $stmt -> execute([$id]);
        ...
        public function getTasksByTagId ($tag_id, $user_id) {
            try {
               $stmt = $this -> db -> prepare('SELECT * FROM todo_list JOIN task_tags ON todo_list.id = task_tags.task_id 
                                                WHERE task_tags.tag_id = :tag_id AND todo_list.user_id = :user_id
                                                ORDER BY ABS(TIMESTAMPDIFF(SECOND, NOW(), finish_date))');
